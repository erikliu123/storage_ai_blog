...
import { asplos2025Papers, asplos2025Sessions, type PaperData } from '@/data/asplos2025'
...
export default function Asplos2025() {
  const highlightCount = asplos2025Papers.filter(p => p.highlight).length
  const diagramCount = asplos2025Papers.filter(p => p.archDiagram).length
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10 animate-fade-in">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-2xl font-bold mb-2 gradient-text">ASPLOS 2025 论文解读专栏</h1>
        <p className="text-sm text-muted-foreground mb-4">
          International Conference on Architectural Support for Programming Languages and Operating Systems — 体系结构与系统顶级会议
        </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
        {[
          { label: 'Architecture', count: asplos2025Papers.filter(p => p.session === 'Architecture and Systems').length, color: 'var(--tag-storage)' },
          { label: 'ML Systems', count: asplos2025Papers.filter(p => p.session === 'Machine Learning Systems').length, color: 'var(--tag-ai)' },
          { label: 'Storage & Memory', count: asplos2025Papers.filter(p => p.session === 'Storage and Memory').length, color: 'var(--tag-ssd)' },
          { label: 'Security', count: asplos2025Papers.filter(p => p.session === 'Security and Reliability').length, color: 'var(--muted-foreground)' },
        ].map(stat => (
      ...
      {/* Papers by session */}
      {asplos2025Sessions.map(session => {
        const sessionPapers = asplos2025Papers.filter(p => p.session === session)
        return (
      ...
      <div className="text-center pt-6 border-t border-border">
        ...
        <a
          href="https://dblp.uni-trier.de/db/conf/asplos/asplos2025.html"
          target="_blank"
