...
import { osdi2025Papers, osdi2025Sessions, type PaperData } from '@/data/conferences'
...
export default function Osdi2025() {
  const highlightCount = osdi2025Papers.filter(p => p.highlight).length
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-10 animate-fade-in">
      <div className="mb-10">
        ...
        <h1 className="text-2xl font-bold mb-2 gradient-text">OSDI 2025 论文解读专栏</h1>
        <p className="text-sm text-muted-foreground mb-4">
          USENIX Symposium on Operating Systems Design and Implementation — 操作系统顶级会议
        </p>
        <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" />2025年7月</span>
          <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" />Boston, USA</span>
          <span className="flex items-center gap-1.5"><BookOpen className="w-3.5 h-3.5" />{osdi2025Papers.length} 篇精选论文</span>
          <span className="tag-storage rounded-full px-2 py-0.5">{highlightCount} 篇重点解读</span>
        </div>
      </div>

      {osdi2025Sessions.map(session => {
        const sessionPapers = osdi2025Papers.filter(p => p.session === session)
        return (
      ...
      <div className="text-center pt-6 border-t border-border">
        <p className="text-xs text-muted-foreground font-mono">数据来源：USENIX OSDI 2025 · 论文解读持续更新中</p>
        <a href="https://www.usenix.org/conference/osdi25" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 mt-3 text-xs text-primary hover:underline">
          <ExternalLink className="w-3 h-3" />查看 OSDI 2025 官网
        </a>
      </div>
    </main>
  )
